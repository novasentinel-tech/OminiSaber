(() => {
  ...
  const saveManagerDescriptorsBatch=async descriptors=>{await guard();const{data,error}=await db.rpc('salvar_descritores_curriculares_lote',{p_descritores:descriptors});if(error)throw error;return data};
  const updateManagerDescriptor=async(id,descriptor)=>{await guard();const{data,error}=await db.rpc('atualizar_descritor_curricular',{p_descritor_id:id,p_descritor:descriptor});if(error)throw error;return data};
